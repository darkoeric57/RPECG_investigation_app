...
// ─── Colour constants ──────────────────────────────────────────────────────────
const _kPrimary = Color(0xFFD97706); // Amber 600
const _kPrimaryLight = Color(0xFFF59E0B); // Amber 500
...
// ─────────────────────────────────────────────────────────────────────────────
class ConsumptionReportPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final filteredLedgers = ref.watch(filteredConsumptionLedgersProvider);
    final isExporting = ref.watch(consumptionExportLoadingProvider);
    final rawRowsCount = ref.watch(consumptionRawRowsProvider).length;
...
class _ActiveReportBanner extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final switchableReports = allReports.where((r) => 
      (r.config.type == ReportType.consumption) &&
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: activeReport != null ? const Color(0xFFFEF3C7) : Colors.white, // Amber 100
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: activeReport != null ? const Color(0xFFFDE68A) : _kBorder, // Amber 200
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: Icon(
              activeReport != null ? Icons.schedule_send_rounded : Icons.bolt_rounded,
              color: activeReport != null ? const Color(0xFFD97706) : _kPrimary,
              size: 22,
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Text(
                      activeReport != null ? 'FILTERED BY SCHEDULE' : 'CONSUMPTION SCOPE',
                      style: TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w900,
                        color: activeReport != null ? const Color(0xFFB45309) : const Color(0xFF64748B),
                        letterSpacing: 1.2,
                      ),
                    ),
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<ActiveReport?>(
                ...
                onChanged: (selected) {
                  ref.read(activeConsumptionReportProvider.notifier).state = selected;
                },
                items: [
                      ...
                      child: Row(
                        children: [
                          Icon(
                            ...
                            size: 16,
                            color: r.status == ActiveReportStatus.draftReady ? const Color(0xFFD97706) : const Color(0xFF10B981),
                          ),
                          const SizedBox(width: 8),
                          Text(
                            r.title,
          ...
          if (activeReport != null) ...[
            ...
            TextButton.icon(
              onPressed: () {
                ref.read(activeConsumptionReportProvider.notifier).state = null;
              },
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Analyzing meter readings and computing active usage profiles.',
...
class _Header extends ConsumerWidget {
      ...
      children: [
        Column(
          ...
          children: [
            ...
            const Text('Consumption Analysis Report',
                style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: Color(0xFF0F172A), letterSpacing: -1)),
            const SizedBox(height: 4),
            const Text('Consumption breakdown, usage patterns, and load profiling statistics.',
  ...
  Future<void> _export(BuildContext context, WidgetRef ref, String format) async {
    final sumAsync = ref.read(consumptionAnalysisSummaryProvider);
    final sum = sumAsync.valueOrNull;
    if (sum == null) return;
    ref.read(consumptionExportLoadingProvider.notifier).state = true;
    try {
      final date = DateFormat('yyyyMMdd_HHmm').format(sum.generatedAt);
      if (format == 'csv') {
        final bytes = RevenueReportExporter.toConsumptionCSV(sum);
        WebUtils.downloadBytes('consumption_analysis_$date.csv', bytes, 'text/csv');
      } else if (format == 'excel') {
        final bytes = RevenueReportExporter.toConsumptionExcel(sum);
        WebUtils.downloadBytes('consumption_analysis_$date.xlsx',
            bytes, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      } else {
        final bytes = await RevenueReportExporter.toConsumptionPDF(sum);
        WebUtils.downloadBytes('consumption_analysis_$date.pdf', bytes, 'application/pdf');
      }
      if (context.mounted) {
      ...
      ref.read(consumptionExportLoadingProvider.notifier).state = false;
    }
  }
}
...
class _KpiRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      Expanded(child: _KpiCard(label: 'TOTAL CONSUMPTION', value: '${grandTotalConsumption.toStringAsFixed(1)} kWh', icon: Icons.bolt_rounded, color: _kPrimary)),
      const SizedBox(width: 20),
      Expanded(child: _KpiCard(label: 'AVERAGE CONSUMPTION', value: '${grandAvgConsumption.toStringAsFixed(1)} kWh', icon: Icons.analytics_rounded, color: _kPrimaryLight, sub: 'Average usage per account')),
      const SizedBox(width: 20),
      Expanded(child: _KpiCard(label: 'PEAK CONSUMPTION', value: '${grandMaxConsumption.toStringAsFixed(1)} kWh', icon: Icons.trending_up_rounded, color: _kOutstanding, sub: 'Highest recorded cycle')),
      const SizedBox(width: 20),
      Expanded(child: _KpiCard(label: 'ACTIVE METERS', value: '$activeAccounts', icon: Icons.flash_on_rounded, color: _kSettled, sub: 'Accounts with usage > 0')),
      const SizedBox(width: 20),
      Expanded(child: _KpiCard(label: 'MISMATCH METERS', value: '${summary.mismatchAccounts}', icon: Icons.warning_amber_rounded, color: _kMismatch, sub: 'Potential audit required')),
    ]);
  }
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Row(children: [
      ...
      Container(
        ...
        child: TextField(
          onChanged: (v) => ref.read(consumptionSearchQueryProvider.notifier).state = v,
              ...
              return Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    ...
                    ref.read(consumptionStatusFilterProvider.notifier).state = next;
                  },
      ...
      if (summary != null)
        Text('${ref.watch(filteredConsumptionLedgersProvider).length} of ${summary!.totalAccounts} accounts',
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.bolt_outlined, size: 36, color: _kPrimary),
            ),
            const SizedBox(height: 24),
            const Text(
              'No Consumption Analysis Executed',
              style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF0F172A)),
            ),
            const SizedBox(height: 8),
            const Text(
              'Select an active report schedule configured for Consumption from the schedule pipeline above to run sequential meter usage analysis.',
