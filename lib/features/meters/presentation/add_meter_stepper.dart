...
class AddMeterStepper extends ConsumerWidget {
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: isCompleted || isActive ? AppTheme.secondary : Colors.grey.shade200,
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        fontSize: 12, 
                        fontWeight: FontWeight.bold, 
                        color: state.capturedImagePaths.length >= 5 ? AppTheme.accent : AppTheme.primary
                      ),
                    ),
                  ],
                ),
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      if (index < state.capturedImagePaths.length) {
                        return _buildImageThumbnail(context, state.capturedImagePaths[index], index, notifier);
                      } else {
                ...
                _buildUploadCard(
                  ...
                  onRemove: notifier.captureVideo, // Changed from removeVideo to captureVideo for "perfect" retake
                  isVideo: true,
                ),
                const SizedBox(height: 24),
                CustomButton(
                  text: state.estimatedLoadWatts != null 
                      ...
                      : 'Add Load Details',
                  icon: state.estimatedLoadWatts != null ? Icons.check_circle : Icons.add_circle_outline,
  ...
  Widget _buildImageThumbnail(BuildContext context, String path, int index, AddMeterNotifier notifier) {
        ...
        child: Container(
          ...
          child: const Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text('Add Photo', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.primary)),
            ],
          ),
        ),
      ),
  ...
  Widget _buildStep3(BuildContext context, WidgetRef ref) {
    ...
    return Column(
      ...
      children: [
        ...
        const Text('Please verify the meter installation details.', style: TextStyle(fontSize: 14, color: AppTheme.textLight)),
        const SizedBox(height: 24),
        _buildReviewCard(
              ...
              children: [
                ...
                const Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.map_outlined, color: AppTheme.primary, size: 40),
                      SizedBox(height: 8),
                      Text('Satellite View Preview', style: TextStyle(fontSize: 12, color: AppTheme.textLight, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildToggleField(String label, List<String> options, String selectedValue, Function(String) onSelected, {bool small = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          ...
          child: Text(
            label.toUpperCase(),
            style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.textLight, letterSpacing: 1.2),
          ),
        ),
        Container(
          padding: const EdgeInsets.all(4),
  ...
  Widget _buildDropdownField(String label, List<String> items, String selectedValue, Function(String) onSelected) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
  ...
  Widget _buildUploadCard(BuildContext context, IconData icon, String label, {String? path, VoidCallback? onTap, VoidCallback? onRemove, bool isVideo = false}) {
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          image: (hasCapture && !isVideo) 
            ? DecorationImage(image: FileImage(File(path)), fit: BoxFit.cover, opacity: 0.6) 
            : null,
        ),
        child: Stack(
          children: [
            if (hasCapture) ...[
              Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(isVideo ? Icons.play_circle_fill : Icons.check_circle, color: Colors.white, size: 40),
                    const SizedBox(height: 8),
                    Text(
                  ...
                  child: Container(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 4),
                        Text('RETAKE', style: TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                ),
              ...
              Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      label, 
                      style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppTheme.textDark, letterSpacing: 0.5),
                    ),
  ...
  Widget _buildReviewCard({required IconData icon, required String title, List<Widget>? rows, Widget? content}) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        color: Colors.white,
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            ...
            child: Text(
              label.toUpperCase(),
              style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: AppTheme.textLight, letterSpacing: 0.5),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
  ...
  Widget _buildBottomActions(BuildContext context, WidgetRef ref, AddMeterState state, AddMeterNotifier notifier) {
    return Container(
      padding: const EdgeInsets.fromLTRB(24, 20, 24, 32),
      decoration: BoxDecoration(
        color: AppTheme.backgroundLight.withOpacity(0.95),
        ...
        children: [
          ...
          Expanded(
            ...
            child: CustomButton(
              ...
              onPressed: () {
                bool isValid = true;
                if (state.currentStep == 0 && !state.isStep1Valid) isValid = false;
                if (state.currentStep == 1 && !state.isStep2Valid) isValid = false;

                if (!isValid) {
                   ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Please fill all required fields and capture an image.')),
                  );
                  return;
                }
                ...
                if (state.currentStep < 2) {
                  ...
                  final meter = Meter(
                    ...
                    tariffActivity: state.tariffActivity == 'Residential' 
                        ...
                        : (state.tariffActivity == 'Commercial' ? TariffActivity.commercial : TariffActivity.industrial),
                    geocode: state.geocode,
                    spnNumber: state.spnNumber,
                    brand: state.meterBrand,
                    rating: state.meterRating,
                    phase: state.meterPhase == '1-Phase' ? MeterPhase.single : MeterPhase.three,
                    type: state.meteringType == 'Prepaid' ? MeteringType.prepaid : MeteringType.postpaid,
  ...
  Future<void> _showSettingsRedirectDialog(BuildContext context, WidgetRef ref) {
    return showDialog(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
